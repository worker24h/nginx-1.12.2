...
struct ngx_event_s
{
     ...
     * 该字段表示当前事件是否有效。
     * 比如一次epoll通知可能需要处理多个连接的事件，例如下场景出现:
     * 需要处理A,B,C三个事件，在处理A的时候，C事件对应的连接关闭了(tcp closed)，
     * 那么C事件相当于是无效的，因此instance应该设置为0
     */
    unsigned instance : 1;
    ...
    ngx_event_handler_pt handler; /* 处理该事件的回调函数 */
...
#elif (NGX_HAVE_EPOLL) && !(NGX_TEST_BUILD_EPOLL) //epoll模型下定义的宏值
