...
ngx_atomic_t *ngx_accept_mutex_ptr; /* 原子变量 实际指向共享内存 */
ngx_shmtx_t ngx_accept_mutex; /* 互斥锁--如果os支持原子锁则采用原子锁否则采用文件锁 */
...
/**
 * 默认值是负数。当worker进程处理一个accept事件则进行++。 
 * 当是正数时，则不能处理accept事件，也就不能进行抢占锁操作
 */
ngx_int_t ngx_accept_disabled;

/* nginx 统计模块 必须在configure过程中指定--with-http_stub_status_module */
#if (NGX_STAT_STUB)
